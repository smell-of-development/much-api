...
public interface AuthApi {
    ...
    @Operation(summary = "로그인",
            description = """
                    ...
                    - password : 비밀번호
                    ### 응답값
                    - 토큰발급(responseCode: 200) : id, accessToken, refreshToken
                    """,
    ...
    @Operation(summary = "OAuth2 정보 처리",
            description = """
                    ...
                    ### 케이스별 결과값
                    1. 최초 사용자
                    - 휴대폰번호 존재(responseCode: 8100)   : id => 추가정보 등록 필요
                    - 휴대폰번호 미존재(responseCode: 8101) : id => 추가정보 + 문자인증 필요.
                    2. 기존 사용자 / 기존 사용자와 휴대폰번호가 같은 최초 사용자
                    - 토큰발급(responseCode: 200) : id, accessToken, refreshToken
                    """,
            parameters = {
                    @Parameter(name = "provider", description = "kakao 또는 google"),
                    @Parameter(name = "responseCode", description = "redirect uri 쿼리스트링에 포함된 responseCode")
            })
    ResponseEntity<Envelope<Jwt>> handleOAuth2(String provider, String responseCode);
    ...
    @Operation(summary = "액세스토큰 갱신",
            description = """
                    ...
                    ### 응답값
                    - responseCode 200  : accessToken - 리프레시 된 액세스 토큰
                    - responseCode 1000 : 요청값 이상 (accessToken, refreshToken 모두 필수)
                    - responseCode 4000 : 갱신불가
                    """,
            requestBody = @RequestBody(required = true))
    ...
    @Operation(summary = "로그인 체크",
            description = """
                    ...
                    ### 응답값
                    - responseCode 200  : 정상토큰 ex) result: 1 (사용자의 id)
                    - responseCode 4000 : 토큰이 없거나 비정상
                    """)
    ...
    @Operation(summary = "가입 SMS 인증번호 발송",
            description = """
                    ...
                    ### 응답값
                    - responseCode 200  : phoneNumber - 휴대폰번호, remainTimeInSeconds - 남은시간(초)
                    - responseCode 1000 : phoneNumber 파라미터가 없음
                    - responseCode 8000 : 휴대폰 번호 중복
                    - responseCode 8001 : 휴대폰번호 형식이 아님
                    - responseCode 8002 : 메세지 발송 실패
                    """,
            parameters = {
    ...
    @Operation(summary = "가입 SMS 인증번호 확인",
            description = """
                    ...
                    ### 응답값
                    - responseCode 200  : 인증성공
                    - responseCode 1000 : 요청값이 올바른 형태가 아님
                    - responseCode 8001 : 휴대폰번호 형식이 아님
                    - responseCode 8003 : 인증번호가 일치하지 않음
                    - responseCode 8004 : 발송기록을 찾을 수 없음
                    """,
